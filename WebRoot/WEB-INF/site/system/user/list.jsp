...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont"></i> 首页 <span class="c-gray en">&gt;</span>系统管理 <span class="c-gray en">&gt;</span>用户列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			...
			<a href="javascript:;" onclick="add('添加用户','toIns.action','893','480')" class="btn btn-primary radius">
				<i class="Hui-iconfont">&#xe600;</i> 添加用户
			</a>
		</span>
		<div class="r">
			姓名:<input type="text" class="input-text" style="width:200px" placeholder="用户姓名" id="search" />&nbsp;&nbsp;
...
$(function() {
	datatable = $('.table-sort').DataTable({
		...
		columns: [{
			...
			defaultContent: "",
			render: function(data, type, row, meta) {
				return '<a class="view" onclick="edit_show(\'查看信息\',\'viewEmployee.action\',' + row.id + ',\'360\',\'500\')">' + data + '</a>';
			}
		}, {
			data: "loginName",
			defaultContent: "",
			...
			orderable:false,
			render: function(data, type, row, meta) {
				return '<a class="view" onclick="edit_show(\'密码重置\',\'resetPassword.action\',' + row.id + ',\'893\',\'200\')">重置</a>';
			},
		}, {
			data: "state",
			render: function(data, type, row, meta) {
			...
			render: function(data, type, row, meta) {
				var a = "";
				a += '<a title="' + state[data] + '" style="text-decoration:none" onClick="changeStatus(' + "'" +  state[data] + "'" + ',[' + "'" + row.userName + "'" + '],\'updateEmployeeState.action\',' + row.id + ','+ flag[data] + ')" href="javascript:;"><i class="Hui-iconfont">' + icon[data] + '</i></a>';
				a += '<a title="编辑" href="javascript:;" onclick="edit_show([' + "'" + row.userName + "'" + '],\'toUpdEmpPage.action\',' + row.id + ',\'893\',\'420\')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>';
